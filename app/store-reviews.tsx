...
export default function StoreReviewsScreen() {
  ...
  const storeName = name as string;

  const [showRatingModal, setShowRatingModal] = useState(false);

  const { data: reviewsData, isLoading } = useQuery(
    trpc.reviews.getStoreReviews.queryOptions({ storeId }),
  );
    ...
    <>
      ...
      <View style={styles.container}>
        ...
        <View style={styles.summaryCard}>
          <Text style={styles.entityName}>{storeName}</Text>
          ...
          >
            {reviews.length === 0 ? (
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptySubText}>كن أول من يقيم هذا المتجر</Text>
              </View>
            ) : (
