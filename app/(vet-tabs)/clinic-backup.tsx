...
import { Building2, Calendar, Plus, Users, Clock, MapPin, Phone, Star, UserPlus, Settings } from 'lucide-react-native';
...
// Mock data for assigned poultry farms
const mockAssignedFarms = [
  {
    id: '1',
...
export default function VetClinicScreen() {
  ...
  const handleAssignDoctor = () => {
    router.push('/assign-poultry-doctor');
  };

  const handleManageFarm = (farmId: string) => {
    router.push({
      pathname: '/manage-poultry-farm',
      params: { farmId }
    });
  };
          ...
          <View>
            ...
            {true && (
              <View style={styles.adminSection}>
                <Text style={[styles.sectionTitle, { textAlign: isRTL ? 'right' : 'left', marginBottom: 16 }]}>
                  إدارة حقول الدواجن
                </Text>
            ...
            {/* Assigned Farms - Show for doctors */}
            {mockAssignedFarms.length > 0 && (
              <View style={styles.farmsSection}>
                <Text style={[styles.sectionTitle, { textAlign: isRTL ? 'right' : 'left', marginBottom: 16 }]}>
                  حقول الدواجن المعينة
                </Text>
                
                {mockAssignedFarms.map((farm) => (
                  <TouchableOpacity
                    key={farm.id}
                    style={styles.farmCard}
                    onPress={() => handleManageFarm(farm.id)}
                  >
                    <View style={[styles.farmContent, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
                      ...
                      <View style={[styles.farmInfo, { flex: 1, marginLeft: isRTL ? 0 : 12, marginRight: isRTL ? 12 : 0 }]}>
                        <Text style={[styles.farmName, { textAlign: isRTL ? 'right' : 'left' }]}>
                          إدارة حقل {farm.owner}
                        </Text>
                        <Text style={[styles.farmSubtitle, { textAlign: isRTL ? 'right' : 'left' }]}>
                          {farm.name}
                        </Text>
                        <View style={[styles.farmDetailRow, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
                          <MapPin size={14} color={COLORS.darkGray} />
                          <Text style={[styles.farmDetailText, { marginLeft: isRTL ? 0 : 4, marginRight: isRTL ? 4 : 0 }]}>
                            {farm.location}
                          </Text>
                        </View>
                        <Text style={[styles.farmLastVisit, { textAlign: isRTL ? 'right' : 'left' }]}>
                          آخر زيارة: {farm.lastVisit}
                        </Text>
                      </View>
                      
                      {farm.alerts > 0 && (
                        <View style={styles.alertBadge}>
                          <Text style={styles.alertBadgeText}>{farm.alerts}</Text>
                        </View>
                      )}
                      
                      <View style={styles.farmArrow}>
