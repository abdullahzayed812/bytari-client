...
import { Stethoscope, Search, UserCheck, UserX, Eye, Phone, Mail, Calendar, MessageCircle, BadgeCheck, Star, X } from "lucide-react-native";
...
export default function AdminVeterinariansList() {
  const { user: currentUser } = useApp();
  const currentAdminId = currentUser?.id ? Number(currentUser.id) : 0;

  const [searchQuery, setSearchQuery] = useState("");
  const [showMessageModal, setShowMessageModal] = useState(false);
  const [messageRecipient, setMessageRecipient] = useState<VetRow["user"] | null>(null);
  ...
  const handleBanUser = (u: VetRow["user"]) => {
    const action = u.isActive ? "حظر" : "إلغاء حظر";
    Alert.alert(`${action} الطبيب`, `هل أنت متأكد من ${action} ${u.name}؟`, [
        ...
        onPress: () => {
          banUserMutation.mutate(
            { userId: u.id, adminId: currentAdminId, ban: u.isActive, reason: `${action} من قبل المشرف` } as any,
            {
              onSuccess: () => {
                refetch();
                Alert.alert("نجح", "تم تحديث حالة الطبيب بنجاح");
              },
              onError: (err) => Alert.alert("خطأ", err.message || "فشل في تحديث حالة الطبيب"),
            },
          );
        },
      },
  ...
  const handleDeleteUser = (u: VetRow["user"]) => {
    Alert.alert("حذف الطبيب", `هل أنت متأكد من حذف ${u.name}؟ هذا الإجراء لا يمكن التراجع عنه.`, [
      { text: "إلغاء", style: "cancel" },
      {
        text: "حذف",
        style: "destructive",
        onPress: () => {
          deleteUserMutation.mutate(
            { userId: u.id, adminId: currentAdminId, reason: "حذف من قبل المشرف" } as any,
            {
              onSuccess: () => {
                refetch();
                Alert.alert("نجح", "تم حذف الطبيب بنجاح");
              },
              onError: (err) => Alert.alert("خطأ", err.message || "فشل في حذف الطبيب"),
            },
          );
        },
      },
  ...
  const handleViewProfile = (u: VetRow["user"]) => {
    router.push({
  ...
  const renderVetCard = ({ item }: { item: VetRow }) => {
    const { user: u, vet } = item;
    return (
            ...
            <View style={styles.badgesRow}>
              {vet?.isVerified ? (
                <View style={[styles.badge, { backgroundColor: "#E8F5E9" }]}>
                  <BadgeCheck size={12} color="#4CAF50" />
                  <Text style={[styles.badgeText, { color: "#4CAF50" }]}>موثق</Text>
                </View>
              ) : (
                <View style={[styles.badge, { backgroundColor: "#FFF3E0" }]}>
                  <Text style={[styles.badgeText, { color: "#FF9800" }]}>غير موثق</Text>
                </View>
              )}
              {!!vet?.rating && (
                <View style={[styles.badge, { backgroundColor: "#FFF8E1" }]}>
                  <Star size={12} color="#FFB300" />
                  <Text style={[styles.badgeText, { color: "#FFB300" }]}>{vet.rating.toFixed(1)}</Text>
                </View>
              )}
            </View>
          </View>
          ...
          {!!vet?.specialization && (
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailText}>
                {vet.specialization}
                {vet.experience ? ` · ${vet.experience} سنوات خبرة` : ""}
              </Text>
            </View>
          )}
          <View style={styles.detailRow}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Stack.Screen options={{ title: "الأطباء البيطريون", headerStyle: { backgroundColor: "#8E44AD" }, headerTintColor: "#fff" }} />
      ...
      <View style={styles.statsContainer}>
        <Stethoscope size={20} color="#8E44AD" />
        <Text style={styles.statsText}>{vets.length} طبيب بيطري</Text>
      </View>
      ...
      {isLoading ? (
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyText}>لا يوجد أطباء بيطريون</Text>
        </View>
      ) : (
        <FlatList
          data={vets}
          keyExtractor={(item) => `vet-${item.user.id}`}
...
const styles = StyleSheet.create({
  ...
  badgesRow: { flexDirection: "row-reverse", gap: 6, flexWrap: "wrap" },
