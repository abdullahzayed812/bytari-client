...
export default function HomeBooksManagementScreen() {
  ...
  const books = useMemo(() => (vetBooksData as any)?.books, [vetBooksData]);

  const queryClient = useQueryClient();

  // Toggle visibility mutation
  const toggleVisibilityMutation = useMutation(trpc.content.toggleBookHomeVisibility.mutationOptions());

  // Delete mutation
  const deleteBookMutation = useMutation(trpc.content.deleteBook.mutationOptions());
  ...
  const handleDeleteBook = (bookId: string) => {
    Alert.alert("حذف الكتاب", "هل أنت متأكد من حذف هذا الكتاب؟", [
  ...
  const handleImportBooks = () => {
    ...
    const availableBooks = mockVetBooks.filter((book) => !books.some((homeBook) => homeBook.id === book.id));
  ...
  if (vetBooksLoading) return <ActivityIndicator size="large" />;

  const visibleBooks = books?.filter((book) => book.isPublished);
  ...
  const renderBookCard = (book: Book, isVisible: boolean) => (
    <TouchableOpacity key={book.id} style={styles.bookCard} onPress={() => handleBookCardPress(book.id)}>
      ...
      <View style={styles.bookActions}>
        ...
        <TouchableOpacity style={[styles.actionButton, styles.editButton]} onPress={() => handleEditBook(book.id)}>
          <Edit3 size={16} color={COLORS.white} />
          <Text style={styles.actionButtonText}>تعديل</Text>
        </TouchableOpacity>

        <TouchableOpacity style={[styles.actionButton, styles.deleteButton]} onPress={() => handleDeleteBook(book.id)}>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerSubtitle}>تحكم في الكتب التي تظهر في الصفحة الرئيسية</Text>
        </View>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>الكتب المعروضة في الصفحة الرئيسية ({visibleBooks?.length})</Text>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>الكتب المخفية ({hiddenBooks?.length})</Text>
