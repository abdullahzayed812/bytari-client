...
import {
  ...
  PermissionsModal,
  ServicesModal,
} from "../components/clinic-settings-modals";
...
export default function ClinicSettings() {
  ...
  const {
    ...
    trpc.clinics.settings.getSettings.queryOptions({
  ...
  const { data: staffData, refetch: refetchStaff } = useQuery(
    trpc.clinics.settings.staff.getStaff.queryOptions({
      clinicId: Number(clinicId),
    })
  );

  // Mutations
  const updateBasicInfoMutation = useMutation(trpc.clinics.settings.updateBasicInfo.mutationOptions());
  const updateContactInfoMutation = useMutation(trpc.clinics.settings.updateContactInfo.mutationOptions());
  const updateWorkingHoursMutation = useMutation(trpc.clinics.settings.updateWorkingHours.mutationOptions());
  const updateServicesMutation = useMutation(trpc.clinics.settings.updateServices.mutationOptions());
  ...
  // Handle working hours save
  const handleServicesSave = (services: any) => {
    updateServicesMutation.mutate(
      ...
      {
        onSuccess: () => {
          Alert.alert("نجح", "تم تحديث خدمات العيادة");
  ...
  const handleSavePermissions = (permissions: any) => {
    // TODO: Add mutation for updating permissions
    ...
    updatePermissionsMutation.mutate(
      { clinicId: Number(clinicId), permissions },
      {
        onSuccess: () => {
          Alert.alert("نجح", "تم إضافة صلاحية جيدة");
          refetchStaff();
        },
        onError: (error) => {
          Alert.alert("خطأ", error.message);
      ...
      items: [
        {
          ...
          icon: MapPin,
          onPress: () => handleSettingPress("clinic-info"),
        },
        {
          id: "contact-info",
        ...
        },
        {
          id: "services",
          title: "خدمات العيادة",
          subtitle: "الخدمات التي تقدما العيادة",
          icon: Building,
          ...
          <View style={styles.clinicCard}>
            <View style={styles.clinicHeader}>
              ...
              <View style={styles.clinicInfo}>
                <Text style={styles.clinicName}>{clinic?.name || "عيادة"}</Text>
                <Text style={styles.clinicStatus}>{subscription?.isActive ? "نشط" : "غير نشط"} - Premium</Text>
              </View>
            </View>
          </View>
      ...
      {clinic && (
        <>
          ...
          <WorkingHoursModal
            ...
            initialData={clinic.workingHours}
          ...
          <ServicesModal
            ...
            initialData={clinic.services}
            onSave={handleServicesSave}
            isLoading={updateServicesMutation.isPending}
          />
        </>
      )}
