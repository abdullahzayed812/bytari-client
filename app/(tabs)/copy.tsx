...
import { mockClinics, mockPets, mockPoultryFarm } from "@/mocks/data";
import { trpc } from "@/lib/trpc";

export default function PetsScreen() {
  const { t, isRTL } = useI18n();
  const { pets, userMode, user, hasAdminAccess, isSuperAdmin, isModerator } =
    useApp();

  // Fetch user's own pets
  const userPetsQuery = trpc.pets.getUserPets.useQuery(
    { userId: Number(user?.id) || 0 },
    { enabled: !!user?.id && userMode !== "veterinarian" }
  );

  // Fetch user's own farms
  const userFarmsQuery = trpc.admin.fieldAssignments.getUserFarms.useQuery(
    undefined,
    { enabled: !!user?.id && userMode !== "veterinarian" }
  );
  ...
  const allFieldsQuery =
    trpc.admin.fieldAssignments.getAllFieldsForAdmin.useQuery(
  ...
  const [currentUserSupervisorAssignment, setCurrentUserSupervisorAssignment] =
    useState<{ fieldId: string; fieldName: string } | null>(null);
  ...
  const mockAssignedVetData = {
    ...
    "fieldvet@example.com": {
      name: "د. محمود حسن - مشرف حقل",
      email: "fieldvet@example.com",
      fieldId: "field7",
      fieldName: "مزرعة الدجاج البلدي - ديالى",
    },
    "poultryvet@example.com": {
      name: "د. سامي الإشراف البيطري",
      email: "poultryvet@example.com",
      fieldId: "field9",
      fieldName: "مزرعة الدواجن المكثفة - واسط",
    },
  };
  ...
  const availableFields = allFieldsQuery.data?.map((field: any) => ({
    ...
    {
      id: "field2",
      name: "حقل الماشية المتقدم - البصرة",
      type: "livestock",
      owner: "علي حسن",
    },
    {
      id: "field3",
      name: "مزرعة الدواجن الحديثة - أربيل",
      type: "poultry",
      owner: "فاطمة أحمد",
    },
    {
      id: "field4",
      name: "حقل الأغنام والماعز - الموصل",
      type: "livestock",
      owner: "محمد علي",
    },
    {
      id: "field5",
      name: "مزرعة الدواجن العضوية - النجف",
      type: "poultry",
      owner: "سارة أحمد",
    },
    {
      id: "field6",
      name: "حقل الأبقار الحلوب - كربلاء",
      type: "livestock",
      owner: "حسين علي",
    },
    {
      id: "field7",
      name: "مزرعة الدجاج البلدي - ديالى",
      type: "poultry",
      owner: "زينب محمد",
    },
    {
      id: "field8",
      name: "حقل الجاموس - ميسان",
      type: "livestock",
      owner: "عبدالله حسن",
    },
    {
      id: "field9",
      name: "مزرعة الدواجن المكثفة - واسط",
      type: "poultry",
      owner: "نور الهدى",
    },
    {
      id: "field10",
      name: "حقل الماعز الشامي - الأنبار",
      type: "livestock",
      owner: "خالد عبدالله",
    },
    {
      id: "field11",
      name: "مزرعة الرومي والبط - بابل",
      type: "poultry",
      owner: "مريم حسين",
    },
    {
      id: "field12",
      name: "حقل الأغنام العواسي - صلاح الدين",
      type: "livestock",
      owner: "يوسف محمد",
    },
  ];
  ...
  const allAssignedSupervisors = {
    ...assignedSupervisors,
    ...mockAssignedSupervisorData,
  };

  // Check if current user is assigned as supervisor (using mock data for demo)
  const currentUserSupervisorAssignment_real =
    Object.values(allAssignedSupervisors).find(
      (supervisor) => supervisor.email === user?.email
    ) ||
    (userMode === "veterinarian" && user?.email === "fieldvet@example.com"
      ? mockCurrentUserSupervisorAssignment
      : currentUserSupervisorAssignment);
  ...
  const currentUserAssignment =
    ...
    (userMode === "veterinarian"
      ? mockAssignedVetData["vet@example.com"]
      : null);
  ...
  const userClinicsQuery = trpc.clinics.getUserApprovedClinics.useQuery(
    undefined,
    { enabled: !!user?.id && userMode === "veterinarian" }
  );

  // Fetch user's approved warehouses
  const userWarehousesQuery =
    trpc.warehouses.getUserApprovedWarehouses.useQuery(undefined, {
      enabled: !!user?.id && userMode === "veterinarian",
    });
  ...
  // Mock clinic and warehouse data for demo
  const mockDemoClinic = {
    id: "demo-clinic-1",
  ...
  const allClinics = [...approvedClinics, mockDemoClinic];
  const allWarehouses = [...approvedWarehouses, mockDemoWarehouse];
    ...
    return (
      <View style={styles.container}>
        <ScrollView style={styles.listContent}>
          ...
          {/* Field Supervisor Dashboard - Only for field supervisors */}
          {isAssignedSupervisor && currentUserSupervisorAssignment_real && (
            <View style={styles.sectionContainer}>
              <Text style={styles.sectionTitle}>
                لوحة تحكم المشرف على حقل الدواجن
              </Text>
              <TouchableOpacity
                style={styles.supervisorDashboardCard}
                onPress={() =>
                  router.push({
                    pathname: "/vet-field-management",
                    params: {
                      fieldId: currentUserSupervisorAssignment_real.fieldId,
                      isSupervisor: "true",
                    },
                  })
                }
                activeOpacity={0.8}
              >
                <View style={styles.supervisorDashboardHeader}>
                  <View style={styles.supervisorIconLarge}>
                    <UserCheck size={40} color={COLORS.white} />
                  </View>
                  <View style={styles.supervisorDashboardInfo}>
                    <Text style={styles.supervisorDashboardTitle}>
                      {currentUserSupervisorAssignment_real.fieldName}
                    </Text>
                    <View style={styles.supervisorRoleRow}>
                      <UserCheck size={14} color={COLORS.darkGray} />
                      <Text style={styles.supervisorRoleLabel}>
                        مشرف معين من الإدارة
                      </Text>
                    </View>
                    <View
                      style={[
                    ...
                    >
                      <Text style={styles.statusText}>مشرف نشط</Text>
                    </View>
                  </View>
                </View>

                <View style={styles.supervisorDashboardStats}>
                  <View style={styles.supervisorDashboardStat}>
                    <Feather size={20} color={COLORS.primary} />
                    <Text style={styles.supervisorDashboardStatValue}>
                      2,800
                    </Text>
                    <Text style={styles.supervisorDashboardStatLabel}>
                      طيور تحت الإشراف
                    </Text>
                  </View>
                  <View style={styles.supervisorDashboardStat}>
                    <Activity size={20} color={COLORS.success} />
                    <Text style={styles.supervisorDashboardStatValue}>8</Text>
                    <Text style={styles.supervisorDashboardStatLabel}>
                      زيارات إشرافية
                    </Text>
                  </View>
                  <View style={styles.supervisorDashboardStat}>
                    ...
                    <Text style={styles.supervisorDashboardStatValue}>3</Text>
                    <Text style={styles.supervisorDashboardStatLabel}>
                      تقارير معلقة
                    </Text>
                  </View>
                  <View style={styles.supervisorDashboardStat}>
                    ...
                    <Text style={styles.supervisorDashboardStatValue}>15</Text>
                    <Text style={styles.supervisorDashboardStatLabel}>
                      توصيات مطبقة
                    </Text>
                  </View>
                ...
                <View style={styles.supervisorActions}>
                  <TouchableOpacity
                    style={styles.supervisorActionButton}
                    onPress={() =>
                      router.push({
                  ...
                  >
                    ...
                    <Text style={styles.supervisorActionText}>إدارة الحقل</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[
                      styles.supervisorActionButton,
                      { backgroundColor: "#8B5CF6" },
                    ]}
                    onPress={() =>
                      router.push({
          ...
          {/* Field Assignment Dashboard - Only for field-assigned vets */}
          {isAssignedVet && assignedFieldData && (
            <View style={styles.sectionContainer}>
              <Text style={styles.sectionTitle}>
                لوحة تحكم حقل الدواجن المعين
              </Text>
              <TouchableOpacity
                style={styles.fieldDashboardCard}
                onPress={() =>
                  router.push({
                    pathname: "/vet-field-management",
                    params: { fieldId: currentUserAssignment.fieldId },
                  })
                }
                activeOpacity={0.8}
              >
                <View style={styles.fieldDashboardHeader}>
                  <View style={styles.fieldIconLarge}>
                    <Feather size={40} color={COLORS.white} />
                  </View>
                  <View style={styles.fieldDashboardInfo}>
                    <Text style={styles.fieldDashboardTitle}>
                      {assignedFieldData.name}
                    </Text>
                    <View style={styles.fieldOwnerRow}>
                      <UserCheck size={14} color={COLORS.darkGray} />
                      <Text style={styles.fieldOwnerLabel}>
                        المالك: {assignedFieldData.owner}
                      </Text>
                    </View>
                    <View
                      style={[
                    ...
                    >
                      <Text style={styles.statusText}>طبيب معين</Text>
                    </View>
                  </View>
                </View>

                <View style={styles.fieldDashboardStats}>
                  <View style={styles.fieldDashboardStat}>
                    <Feather size={20} color={COLORS.primary} />
                    <Text style={styles.fieldDashboardStatValue}>3,500</Text>
                    <Text style={styles.fieldDashboardStatLabel}>
                      طيور نشطة
                    </Text>
                  </View>
                  <View style={styles.fieldDashboardStat}>
                    <Activity size={20} color={COLORS.success} />
                    <Text style={styles.fieldDashboardStatValue}>12</Text>
                    <Text style={styles.fieldDashboardStatLabel}>
                      زيارات هذا الشهر
                    </Text>
                  </View>
                  <View style={styles.fieldDashboardStat}>
                    <AlertCircle size={20} color={COLORS.warning} />
                    <Text style={styles.fieldDashboardStatValue}>5</Text>
                    <Text style={styles.fieldDashboardStatLabel}>تنبيهات</Text>
                  </View>
                  <View style={styles.fieldDashboardStat}>
                    <Syringe size={20} color={COLORS.primary} />
                    <Text style={styles.fieldDashboardStatValue}>28</Text>
                    <Text style={styles.fieldDashboardStatLabel}>تطعيمات</Text>
                  </View>
                </View>
                ...
                <View style={styles.fieldActions}>
                  ...
                  >
                    <Activity size={16} color={COLORS.white} />
                    <Text style={styles.fieldActionText}>متابعة الحقل</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[
                  ...
                  >
                    <MessageCircle size={16} color={COLORS.white} />
                    <Text style={styles.fieldActionText}>محادثة المالك</Text>
                  </TouchableOpacity>
                </View>
              </TouchableOpacity>
            </View>
                    ...
                    <View style={styles.warehouseHeader}>
                      ...
                      <View
                        style={[
                          styles.statusBadge,
                          {
                            backgroundColor: getWarehouseStatusColor("active"),
                          },
                        ]}
                      >
                        <Text style={styles.statusText}>مفعل</Text>
          ...
          {allClinics.length === 0 && allWarehouses.length === 0 && (
                    ...
                    <TouchableOpacity
                      ...
                      style={[
                        ...
                        selectedField === field.id &&
                          styles.fieldListOptionSelected,
                      ]}
                      ...
                      <View style={styles.fieldOptionContent}>
                        <Text
                          style={[
                            ...
                            selectedField === field.id &&
                              styles.fieldListOptionTextSelected,
                          ]}
                        >
                          {field.name}
                        </Text>
                        ...
                        >
                          <Text style={styles.fieldTypeBadgeText}>
                            {field.type === "poultry" ? "دواجن" : "ماشية"}
                          </Text>
                        </View>
                      </View>
                    </TouchableOpacity>
                  ...
                  {availableFields
                    ...
                    .map((field: any) => (
                      <TouchableOpacity
                        ...
                        style={[
                          styles.fieldListOption,
                          selectedSupervisorField === field.id &&
                            styles.fieldListOptionSelected,
                        ]}
                ...
                <Button
                  ...
                  onPress={() => {
                    if (
                      supervisorName &&
                      supervisorEmail &&
                      selectedSupervisorField
                    ) {
                      const field = availableFields.find(
                        (f: any) => f.id === selectedSupervisorField
                      );
                      if (field) {
  ...
  const renderPetItem = ({ item }: { item: Pet }) => {
    const upcomingReminders = item.reminders.filter(
      (reminder) =>
        !reminder.isCompleted && new Date(reminder.date) > new Date()
    );
...
const styles = StyleSheet.create({
  ...
  fieldDashboardStatValue: {
    fontSize: 16,
    fontWeight: "bold",
    color: COLORS.black,
    marginTop: 4,
  },
  fieldDashboardStatLabel: {
    fontSize: 10,
    color: COLORS.darkGray,
    marginTop: 2,
    textAlign: "center",
  ...
  supervisorDashboardInfo: {
    flex: 1,
    marginRight: 16,
  },
  supervisorDashboardTitle: {
    fontSize: 18,
    fontWeight: "bold",
    color: COLORS.black,
    marginBottom: 6,
  ...
  supervisorDashboardStat: {
    alignItems: "center",
    flex: 1,
  },
  supervisorDashboardStatValue: {
    fontSize: 16,
    fontWeight: "bold",
    color: COLORS.black,
    marginTop: 4,
  },
  supervisorDashboardStatLabel: {
    fontSize: 10,
    color: COLORS.darkGray,
    marginTop: 2,
    textAlign: "center",
