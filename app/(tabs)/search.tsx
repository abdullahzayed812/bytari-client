  ...
  const handleSearch = (query: string) => {
    ...
    setTimeout(() => {
      ...
      if (userMode === "pet_owner") {
        ...
        if (
          query.toLowerCase().includes("طعام") ||
          query.toLowerCase().includes("food") ||
          query.toLowerCase().includes("منتج") ||
          ...
          results.push({
            ...
            image:
              "https://images.unsplash.com/photo-1589924691995-400dc9ecc119?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2076&q=80",
            description: "السعر: 45 دينار",
          });
          results.push({
            id: "product2",
            type: "product",
            ...
            description: "السعر: 25 دينار",
          });
        }

        // Search in tips
        if (
          query.toLowerCase().includes("نصيحة") ||
          query.toLowerCase().includes("tip") ||
          query.toLowerCase().includes("رعاية") ||
          query.toLowerCase().includes("تربية")
        ) {
          results.push({
            id: "tip1",
            type: "tip",
        ...
        if (
          query.toLowerCase().includes("مذخر") ||
          query.toLowerCase().includes("warehouse") ||
          query.toLowerCase().includes("أدوية") ||
        ...
        // Search in books
        if (
          query.toLowerCase().includes("كتاب") ||
          query.toLowerCase().includes("book") ||
          query.toLowerCase().includes("مرجع") ||
          ...
          results.push({
            ...
            image:
              "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2087&q=80",
        ...
        // Search in articles
        if (
          query.toLowerCase().includes("مقال") ||
          query.toLowerCase().includes("article") ||
          query.toLowerCase().includes("بحث") ||
          query.toLowerCase().includes("دراسة")
        ) {
          results.push({
            id: "article1",
        ...
        if (
          query.toLowerCase().includes("منتج") ||
          query.toLowerCase().includes("product") ||
          query.toLowerCase().includes("أداة") ||
          query.toLowerCase().includes("جهاز")
        ) {
          results.push({
            id: "product3",
            type: "product",
      ...
      <View style={styles.content}>
        ...
        <ScrollView style={styles.resultsContainer} showsVerticalScrollIndicator={false}>
          {searchQuery.length === 0 ? (
            ...
            <View>
              <Text style={[styles.sectionTitle, { textAlign: isRTL ? "right" : "left" }]}>عمليات البحث الأخيرة</Text>
              ...
              <View style={styles.suggestionsContainer}>
                {(userMode === "pet_owner"
                  ...
                  <TouchableOpacity key={index} style={styles.suggestionChip} onPress={() => handleSearch(suggestion)}>
                  ...
                  <View style={[styles.resultContent, { flexDirection: isRTL ? "row-reverse" : "row" }]}>
                    <View style={styles.resultImageContainer}>
                      {result.image ? (
                        ...
                        <View style={styles.resultIconContainer}>{getResultIcon(result.type)}</View>
                      )}
                    </View>
                    ...
                    >
                      <Text style={[styles.resultTitle, { textAlign: isRTL ? "right" : "left" }]}>{result.title}</Text>
                      {result.subtitle && (
