...
export function EggMarketCard({ ad, isAdmin, onDelete, onPress }: EggMarketCardProps) {
  const typeLabel = EGG_TYPE_LABELS[ad.eggType] || ad.eggType;
  const firstImage = ad.images?.[0];
  const unitLabel = ad.unit === "tray" ? "طاقة" : ad.unit === "carton" ? "كرتون" : ad.unit;

  const handleCall = () => ad.contactPhone && Linking.openURL(`tel:${ad.contactPhone}`);
